...
export default function Home() {
  ...
  return (
    <div className="md:h-screen overflow-x-hidden" style={{ perspective: "10px" }}>
      {/* Navbar shifted to the left and made sticky */}
      <div className={`absolute flex mt-2 ${isOpen?"left-[50vw]":"left-[82vw]"} md:left-[65vw] z-[50] bg-transparent pt-1 `}>
        
          <div className={`z-[2] flex flex-col md:flex-row w-max bg-white rounded-lg py-2 px-1 ${isOpen?"block":"hidden"} md:flex`}>
            <div className="p-2 mx-1 cursor-pointer text-sm font-bold rounded-lg hover:bg-gray-200">About</div>
            <div className="p-2 mx-1 cursor-pointer text-sm font-bold rounded-lg hover:bg-gray-200">Experience</div>
            <div className="p-2 mx-1 cursor-pointer text-sm font-bold rounded-lg hover:bg-gray-200">Projects</div>
            <div className="p-2 mx-1 cursor-pointer text-sm font-bold rounded-lg hover:bg-gray-200">Contact</div>
            <div className="p-2 mx-1 cursor-pointer text-sm font-bold rounded-lg bg-black text-white hover:text-blue-300">Resume</div>
          </div>
        <button
      ...
      <div className="h-screen md:h-full md:flex md:justify-center md:items-center z-[-1]">
        ...
        />
        <img src="/template.jpg" className="object-cover w-full h-full md:h-auto md:absolute md:top-[50.1rem] " />
      </div>
      <div className="h-screen md:h-full md:flex md:justify-center md:items-center z-[-1]">
        <img src="/field.jpg" className="object-cover w-full h-full md:h-auto md:absolute md:top-[94.2rem]" />
      </div>
      <div className="h-screen md:h-full md:flex md:justify-center md:items-center z-[-1]">
        <img src="/sakura.jpg" className="object-cover w-full h-full md:h-auto md:absolute md:top-[156.6rem]" />
      </div>
      <div className="h-screen md:h-full md:flex md:justify-center md:items-center z-[-1]">
        <img src="/day.jpg" className="object-cover w-full h-full md:h-auto md:absolute md:top-[209.1rem]" />
      </div>
    </div>
  );
}
